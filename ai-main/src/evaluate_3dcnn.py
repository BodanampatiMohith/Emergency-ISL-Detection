...
def main():
    ...
    for rec in items:
        ...
        if not all(os.path.exists(p) for p in fps):
            continue
        seq = _load_sequence_np(fps)[None, ...]
    ...
    cm_path = os.path.join(REPORTS_DIR, "confusion_matrix_3dcnn.png")
    plt.tight_layout()
    plt.savefig(cm_path)
    plt.close()
    ...
    rep_path = os.path.join(REPORTS_DIR, "evaluation_report_3dcnn.json")
    with open(rep_path, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2)

    print(f"[INFO] Overall accuracy (3D CNN): {acc:.4f}")
