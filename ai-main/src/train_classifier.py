"""Train VGG16+LSTM classifier
- Loads 60/20/20 splits from `data/processed/splits/*.txt`
- Builds tf.data.Dataset from frame folders
- Trains with Adam(lr=1e-3), batch=32
- Saves best model to `data/processed/artifacts/models/vgg16_lstm.h5`
"""
import os
import json
...
from src.models_vgg_lstm import build_vgg16_lstm
...
def main():
    ...
    model = build_vgg16_lstm(trainable_backbone=False)
    opt = optimizers.Adam(learning_rate=LR)
    model.compile(optimizer=opt, loss="categorical_crossentropy", metrics=["accuracy"])

    ckpt_path = os.path.join(MODEL_DIR, "vgg16_lstm.h5")
    cbs = [
        callbacks.ModelCheckpoint(ckpt_path, monitor="val_accuracy", save_best_only=True, verbose=1),
        callbacks.EarlyStopping(monitor="val_loss", patience=8, restore_best_weights=True),
        callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=4, min_lr=1e-6),
        callbacks.CSVLogger(os.path.join(LOGS_DIR, "training_log.csv")),
    ]
